...
import "../../css/productCategory/productCategory.css";
...
const ProductListPage = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/getProductsByCategory?category=${category}`);
    ...
    return (
        <>
            <Helmet>
                <title>Danh mục {category} | Hải Nam Mobile</title>
                <meta
                    name="description"
                    content={`Tổng hợp sản phẩm thuộc danh mục ${category}, chính hãng, giá tốt tại Hải Nam Mobile.`}
                />
            </Helmet>
                            ...
                            currentData.map((product) => {
                                ...
                                return (
                                    <div key={product._id} className="product-item">
                                        ...
                                        />
                                        <h3>{product.name}</h3>
                                        {rom?.isDiscount == false ? (<div className="proloop-price">
                                            ...
                                            <div className="price-sale">
                                                <div className="price-box">
                                                    <span className="discounted-price">{(rom?.price - rom?.discountPrice).toLocaleString()}đ</span>
                                                    <span className="price">{rom?.price.toLocaleString()}₫</span>
                                                </div>
                                                <span className="discount-percent">-{rom?.discountPercent}%</span>
                                            </div>
                                        )}
