...
import { loadBestSellingProductsAsync } from "../../store/bestSellingProducts.js";
import ProductDetailPreview from "../productDetails/productDetailsPreview";
import axios from "axios";
import "../../css/bestSellingProducts/bestSellingProducts.css";

export default function GetBestSellingProducts() {
    const dispatch = useDispatch();
    const scrollRef = useRef();
    const bestSelling = useSelector((state) => state.bestSellingProducts.BestSellingProducts);
    const [selectedProductId, setSelectedProductId] = useState(null);
    useEffect(() => {
       dispatch(loadBestSellingProductsAsync());
    }, [dispatch]);
            ...
            <div className="product-scroll-container" ref={scrollRef}>
                {bestSelling.map((product) => (
                    <div className="productCard" key={product._id}>
                        {selectedProductId === product._id && <ProductDetailPreview productId={product._id} onClose={() => { setSelectedProductId(null) }} />}
                        <img className="productCard-img" src={`${import.meta.env.VITE_API_BASE_URL}${product?.roms?.[0].variants?.[0]?.image}`} alt={product.name} onClick={() => { setSelectedProductId(product._id) }} />
                        <h3>{product.name}</h3>
                        {product?.roms?.[0]?.isDiscount === false ? (
                            ...
                            <div className="price-sale">
                                <div className="price-box">
                                    <span className="discounted-price">
                                        {(
                                            (product?.roms?.[0]?.price || 0) -
                                            (product?.roms?.[0]?.discountPrice || 0)
                                        ).toLocaleString()}₫
                                    </span>
                                ...
                                <span className="discount-percent">
                                    -{product?.roms?.[0]?.discountPercent}%
                                </span>
                            </div>
                        )}
                        <button className="btnAddToCart" onClick={() => { setSelectedProductId(product._id) }}>
