...
import "../../css/discountedProduct/allDiscountedProduct.css";
import ProductDetailPreview from "../productDetails/productDetailsPreview";
...
const AllDiscountedProducts = () => {
    ...
    useEffect(() => {
        axios
            .get(`${import.meta.env.VITE_API_BASE_URL}/api/getDiscountedProducts`)
    ...
    } = usePagination(discounted, 12);

    return (
        <div className="all-discounted-container">
            <Helmet>
                <title>Tất cả khuyến mãi điện thoại - Hải Nam Mobile</title>
                <meta name="description" content="Tổng hợp các chương trình giảm giá điện thoại hấp dẫn nhất tại Hải Nam Mobile. Nhanh tay mua ngay!" />
                <meta name="keywords" content="khuyến mãi điện thoại, giảm giá, điện thoại giá rẻ, Hải Nam Mobile" />
            </Helmet>
            <h2 className="all-discounted-title">⚡️ TẤT CẢ KHUYẾN MẠI SHOCK 🔥</h2>
            <div className="discounted-grid">
                {paginatedProducts.map((product) => {
                    const rom = product.roms?.[0];
                    const variant = rom?.variants?.[0];
                    return (
                        <div className="discounted-card" key={product._id}>
                            <img
                                src={`${import.meta.env.VITE_API_BASE_URL}${variant?.image}`}
                                alt={product.name}
                                className="discounted-img"
                                onClick={() => setSelectedProductId(product._id)}
                            />
                            <h3 className="discounted-name">{product.name}</h3>
                            <div className="discounted-prices">
                                <div className="discounted-price-box">
                                    <span className="discounted-new">
                                        {(rom?.price - rom?.discountPrice).toLocaleString()}₫
                                    </span>
                                    <span className="discounted-old">
                                        {rom?.price.toLocaleString()}₫
                                    </span>
                                </div>
                                <span className="discounted-percent">-{rom?.discountPercent}%</span>
                            </div>
                            <button className="btn-add-to-cart" onClick={() => { setSelectedProductId(product._id) }}>
