...
export default function ProductDetailPreview({ productId, onClose }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/getProductById/${productId}`);
        setProduct(res.data);

        if (res.data.roms && res.data.roms.length > 0) {
          setSelectedRom(res.data.roms[0]);
          if (res.data.roms[0].variants.length > 0) {
            setSelectedVariant(res.data.roms[0].variants[0]);
          }
        }
      } catch (err) {
  ...
  const handleOpenProductDetailPage = () => {
    navigate(`/product/${product._id}`);
  };

  const handleImageChange = (variant) => {
    document.getElementById("main-image").src = `${import.meta.env.VITE_API_BASE_URL}${variant.image}`;
  };
  ...
  const handleAddToCart = async () => {
    const itemToAdd = {
      ...
      quantity,
      price: selectedRom.isDiscount ? selectedRom.price - selectedRom.discountPrice : selectedRom.price,
      isDiscount: selectedRom.isDiscount || false,
      discountPrice: selectedRom.discountPrice || 0,
    };

    dispatch(addToCartAsync(itemToAdd))
      .unwrap()
      .then(() => toast.success("Thêm sản phẩm vào giỏ hàng thành công!"))
      .catch(() => toast.error("Có lỗi khi thêm giỏ hàng!"));
  };


  if (!product || !selectedRom || !selectedVariant) return <div>Đang tải...</div>;
    ...
    <div className="product-overlay">
      <div className="product-detail-preview">
        <div className="product-image">
          <img
            id="main-image"
            src={`${import.meta.env.VITE_API_BASE_URL}${selectedVariant.image}`}
            alt={product.name}
            className="main-image"
          />
          <div className="image">
            {product.roms[0].variants.map((variant, index) => (
              <img
                key={index}
                src={`${import.meta.env.VITE_API_BASE_URL}${variant.image}`}
                alt={`Color ${variant.color}`}
                className="variant-img"
                onClick={() => handleImageChange(variant)}
              />
            ))}
          </div>
        </div>
        ...
        <div className="product-info-preview">
          <h2 className="head-h2">{product.name}</h2>
          <button className="close-icon" onClick={onClose}>
            <span className="icon-x" />
          </button>
          <div className="selected-rom">
            <strong>Chọn ROM:</strong>
            {product.roms.map((rom, index) => (
              <button className="btn-selected-rom"
                key={index}
                onClick={() => {
                  setSelectedRom(rom);
                  setSelectedVariant(rom.variants[0]);
                }}
                style={{
                  marginLeft: "10px",
                  padding: "6px 12px",
                  backgroundColor: rom.rom === selectedRom.rom ? "#000" : "#eee",
                  color: rom.rom === selectedRom.rom ? "#fff" : "#000",
                }}
              >
                {rom.rom}
              </button>
          ...
          <div className="selected-color" style={{ marginBottom: "12px" }}>
            <strong>Chọn màu:</strong>
            {selectedRom.variants.map((variant, index) => (
              <button className="color-button"
                key={index}
                onClick={() => setSelectedVariant(variant)}
                style={{
                  border: variant.color === selectedVariant.color ? "1px solid black" : "1px solid #ccc",
        ...
        <span className="open-description" onClick={handleOpenProductDetailPage}>xem chi tiết</span>
      </div>
    </div>
  );
}

ProductDetailPreview.propTypes = {
